...
/* ─── Fragment shader: dark navy grid + mesh gradient + ASCII glyphs ─────── */
...
/* ─── ShaderCanvas (adapted from uimix/asd source) ──────────────────────── */
...
/**
 * Renders a fullscreen WebGL2 fragment-shader animation.
 * Exposes iResolution, iTime, iFrame, iMouse uniforms matching the Shadertoy API.
 */
function ShaderCanvas({ fragSource = SHADER_SRC, pixelRatio }: ShaderCanvasProps) {
  ...
  useEffect(() => {
    ...
    if (!glOrNull) return;
    // Capture narrowed type so closures (applySize, tick, cleanup) see non-null gl
...
/* ─── Public: fixed full-page blueprint background ───────────────────────── */

/** Fixed full-screen WebGL2 blueprint-grid background. Mount once at page root. */
