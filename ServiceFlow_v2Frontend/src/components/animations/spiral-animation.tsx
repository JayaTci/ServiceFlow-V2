...
class Star {
  ...
  constructor(cameraZ: number, cameraTravelDistance: number) {
    ...
    this.z = (0.5 * cameraZ) + Math.random() * (cameraTravelDistance + 0.5 * cameraZ - 0.5 * cameraZ);
  ...
  render(p: number, c: AnimationController) {
    ...
    c.showProjectedDot(new Vector3D(vx, vy, this.z), 8.5 * this.strokeWeightFactor * sizeMultiplier);
  }
}
...
class AnimationController {
  ...
  private readonly cycleDuration = 15000;
  ...
  /** Creates 5 000 stars with seeded Math.random for a deterministic base layer. */
  private seedStars() {
  ...
  private tick(timestamp: number) {
    if (this.startTimestamp === null) {
      // Start at 55% into the cycle — skips the slow intro, lands in the active spiral state
      this.startTimestamp = timestamp - 0.55 * this.cycleDuration;
    }
    this.time = ((timestamp - this.startTimestamp) % this.cycleDuration) / this.cycleDuration;
    this.render();
    this.rafId = requestAnimationFrame(this.tick);
  }
  ...
  private rotate(v1: Vector2D, v2: Vector2D, p: number, orientation: boolean): Vector2D {
...
/**
 * Full-viewport spiral animation canvas.
 * Uses ResizeObserver on the wrapper div to size the canvas — avoids the
 * window-state race condition that left element.style empty.
 */
